...
public class FontsContractCompat {
    ...
    public static void requestFont(Context context, FontRequest request, FontRequestCallback callback, Handler handler) {
        ...
        FontRequestWorker.requestFontAsync(context.getApplicationContext(), FontRequestWorker$$ExternalSyntheticBackport1.m(new Object[]{request}), 0, RequestExecutor.createHandlerExecutor(handler), callbackWrapper);
    }
    ...
    public static void requestFont(Context context, FontRequest request, int style, Executor loadingExecutor, Executor callbackExecutor, FontRequestCallback callback) {
        FontRequestWorker.requestFontAsync(context.getApplicationContext(), FontRequestWorker$$ExternalSyntheticBackport1.m(new Object[]{request}), style, loadingExecutor, new CallbackWrapper(callback, callbackExecutor));
    }
    ...
    public static void requestFontWithFallbackChain(Context context, List<FontRequest> requests, int style, Executor loadingExecutor, Executor callbackExecutor, FontRequestCallback callback) {
        FontRequestWorker.requestFontAsync(context.getApplicationContext(), requests, style, loadingExecutor, new CallbackWrapper(callback, callbackExecutor));
    }
    ...
    public static Typeface requestFont(Context context, List<FontRequest> requests, int style, boolean isBlockingFetch, int timeout, Handler handler, FontRequestCallback callback) {
        ...
        if (!isBlockingFetch) {
            return FontRequestWorker.requestFontAsync(context, requests, style, (Executor) null, callbackWrapper);
        }
        if (requests.size() <= 1) {
    ...
    public static class FontInfo {
        ...
        public FontInfo(Uri uri, int ttcIndex, int weight, boolean italic, int resultCode) {
            this(uri, ttcIndex, weight, italic, (String) null, resultCode);
        }
    ...
    public static Typeface getFontSync(Context context, FontRequest request, ResourcesCompat.FontCallback fontCallback, Handler handler, boolean isBlockingFetch, int timeout, int style) {
        ...
        return requestFont(context, (List<FontRequest>) FontRequestWorker$$ExternalSyntheticBackport1.m(new Object[]{request}), style, isBlockingFetch, timeout, newHandler, new TypefaceCompat.ResourcesCallbackAdapter(fontCallback));
    }
